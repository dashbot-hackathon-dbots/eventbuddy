...
sub_classifications = ["hiphop", "jazz", "rock", "pop", "today", "tomorrow", "next weekend", ]
...
class MessageHandler(object):
    ...
    def handle_message(cls, message):
        if message["type"] == "message" or \
                (message["type"] == "conversationUpdate" and
                             set([m["name"] for m in message["membersAdded"]]) - {"Bot"} != set()):

            user_id = message["from"]["id"]
            if message["type"] == "conversationUpdate":
                cls.contexts.set_context(user_id, {})
            context = cls.contexts.get_context(user_id)
            in_message_text = message.get("text", "")
